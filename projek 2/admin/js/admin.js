// admin.js - Main JavaScript untuk Admin Hidup Sehat
...
class AdminApp {
    ...
    loadDashboardData() {
        try {
            const membersData = this.getSampleMembersData();
    ...
    loadMembersData() {
        try {
            this.membersData = this.getSampleMembersData();
            ...
            return this.getSampleMembersData();
        }
    }

    getSampleMembersData() {
        return [
            {
                id: 1,
    ...
    updateMembersStats() {
        ...
        const avgAge = Math.round(this.membersData.reduce((sum, member) => sum + member.age, 0) / this.membersData.length);
        const newThisWeek = this.membersData.length;
    ...
    loadMentalHealthData() {
        ...
        try {
            const membersData = this.loadMembersData();
    ...
    updateMentalHealthStats(members) {
        ...
        this.updateMentalHealthProgress(good, moderate, severe, members.length);
    }
    ...
    updateHealthTable(members) {
        ...
        members.forEach(member => {
            ...
            row.innerHTML = `
                ...
                <td>${member.lastActivity || member.registrationDate}</td>
            `;
    ...
    generateMonthlyData(members) {
        ...
        for (let i = 5; i >= 0; i--) {
            ...
            monthlyCounts[monthKey] = Math.floor(Math.random() * 8) + 2;
        }
...
function contactMember(email) {
    const member = window.adminApp.membersData.find(m => m.email === email);
    if (member) {
        window.adminApp.showNotification(`Menghubungi ${member.fullName} (${member.phone})`, 'info');
    }
}
...
function exportData() {
    if (window.adminApp) {
        window.adminApp.exportData('lengkap');
    }
}
